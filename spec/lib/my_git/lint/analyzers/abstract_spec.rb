...
RSpec.describe MyGit::Lint::Analyzers::Abstract do
  ...
  let :configuration do
    MyGit::Lint::Configuration::Content[
      analyzers: [MyGit::Lint::Configuration::Setting[id: :abstract, enabled: true, severity: :error]]
    ]
  end
  ...
  describe "#enabled?" do
    ...
    context "when disabled" do
      let :configuration do
        MyGit::Lint::Configuration::Content[
          analyzers: [MyGit::Lint::Configuration::Setting[id: :abstract, enabled: false]]
        ]
      end
  ...
  describe "#severity" do
    ...
    context "without severity" do
      let :configuration do
        MyGit::Lint::Configuration::Content[
          analyzers: [MyGit::Lint::Configuration::Setting[id: :abstract]]
        ]
      end

      it "fails with severity error" do
        result = -> { analyzer.severity }
        expect(&result).to raise_error(MyGit::Lint::Errors::Severity, /invalid severity/i)
      end
    end
    ...
    context "with invalid severity" do
      let :configuration do
        MyGit::Lint::Configuration::Content[
          analyzers: [MyGit::Lint::Configuration::Setting[id: :abstract, severity: :bogus]]
        ]
      end

      it "fails with invalid severity error" do
        result = -> { analyzer.severity }
        expect(&result).to raise_error(MyGit::Lint::Errors::Severity, /invalid severity.+bogus/i)
      end
    end
  end
  ...
  describe "#warning?" do
    let :configuration do
      MyGit::Lint::Configuration::Content[
        analyzers: [
          MyGit::Lint::Configuration::Setting[id: :abstract, enabled: true, severity: :warn]
        ]
      ]
    end
