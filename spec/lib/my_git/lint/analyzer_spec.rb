...
RSpec.describe MyGit::Lint::Analyzer do
  ...
  describe "#call" do
    it "answers collector and reporter without block" do
      git_repo_dir.change_dir do
        ...
        expect(runner.call).to contain_exactly(
          kind_of(MyGit::Lint::Collector),
          kind_of(MyGit::Lint::Reporters::Branch)
        )
      end
    end
    ...
    context "with invalid analyzer ID" do
      let :configuration do
        MyGit::Lint::Configuration::Content[
          analyzers: [MyGit::Lint::Configuration::Setting[id: :bogus]]
        ]
      end
